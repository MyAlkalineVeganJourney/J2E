...
const MAVJStore = () => {
  ...
  // FEATURED PRODUCTS - VARIETY (NO SEA MOSS)
  ...
  const violetSeaMoss = {
    ...
    benefit: 'Violet vibration activates crown and third eye chakras - enhances spiritual clarity, amplifies intuitive perception, supports thyroid and pineal gland function, promotes deep sleep cycles, and facilitates quantum coherence with higher consciousness. Substrate-harvested from St. Lucian volcanic reefs - medicinal grade for the educated connoisseur.',
    raw: [
    ...
    gel: [
      products.find(p => p.name.includes('Spectrum') && p.name.includes('Gel 32oz'))
    ].filter(Boolean)
  };

  // CHELATION KIT (CHAPTER 2)
  ...
  return (
    <Layout pageTitle="🛒 MAVJ STORE">
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(0,206,209,0.1), rgba(0,0,0,0.3))', border: '3px solid #00CED1', borderRadius: '20px', padding: 'clamp(30px, 6vw, 50px)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ fontSize: 'clamp(1.1rem, 3vw, 1.4rem)', lineHeight: '1.8', color: '#DDD6B8', marginBottom: '20px' }}>
              Start here. Before anything else, your body needs minerals. Sea Moss was the very first thing I bought outside of myself. It provides approximately 95% of the minerals your body needs daily.
            </p>
            ...
            <div style={{ background: 'rgba(0,0,0,0.8)', border: `3px solid ${violetSeaMoss.color}`, borderRadius: '20px', overflow: 'hidden', boxShadow: `0 10px 40px ${violetSeaMoss.color}50`, marginBottom: '25px' }}>
              ...
              <div style={{ padding: 'clamp(20px, 4vw, 30px)' }}>
                ...
                <div style={{ background: 'rgba(0,206,209,0.1)', padding: 'clamp(15px, 3vw, 20px)', borderRadius: '10px', marginBottom: '20px', border: '2px solid #00CED1' }}>
                  ...
                  <p style={{ color: '#DDD6B8', fontSize: 'clamp(0.85rem, 2vw, 0.95rem)', lineHeight: '1.6' }}>
                    Our gel form delivers the FULL St. Lucia experience - substrate-harvested moss prepared with mineral-rich volcanic spring water from the Pitons. This is medicinal-grade Sea Moss for the educated connoisseur. Know the difference between rope-grown kitchen-grade and substrate-harvested medicinal moss.
                  </p>
                </div>
                
                <div style={{ marginBottom: '20px' }}>
                  <h4 style={{ color: '#FFD700', fontSize: 'clamp(1.1rem, 2.6vw, 1.3rem)', marginBottom: '12px', fontWeight: 'bold' }}>Raw:</h4>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
                    {violetSeaMoss.raw.map((product, i) => product && (
                      <button key={i} onClick={() => handleBuyNow(product.url)} style={{ padding: '12px 8px', background: `${violetSeaMoss.color}20`, border: `2px solid ${violetSeaMoss.color}`, borderRadius: '10px', color: violetSeaMoss.color, fontSize: 'clamp(0.85rem, 2vw, 0.95rem)', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.3s', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                        <span>{product.name.match(/\d+oz/)?.[0]}</span>
                        <span style={{ fontSize: 'clamp(1rem, 2.4vw, 1.2rem)' }}>${product.price}</span>
                      </button>
                    ))}
                  </div>
                </div>

                {violetSeaMoss.gel.length > 0 && (
                  <div style={{ marginBottom: '20px' }}>
                    <h4 style={{ color: '#FFD700', fontSize: 'clamp(1.1rem, 2.6vw, 1.3rem)', marginBottom: '12px', fontWeight: 'bold' }}>Gel (with Volcanic Water):</h4>
                    <div style={{ display: 'grid', gap: '10px' }}>
                      {violetSeaMoss.gel.map((product, i) => product && (
                        <button key={i} onClick={() => handleBuyNow(product.url)} style={{ padding: '12px', background: `${violetSeaMoss.color}20`, border: `2px solid ${violetSeaMoss.color}`, borderRadius: '10px', color: violetSeaMoss.color, fontSize: 'clamp(0.9rem, 2.2vw, 1rem)', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.3s', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span>{product.name.match(/\d+oz/)?.[0]}</span>
                          <span style={{ fontSize: 'clamp(1.1rem, 2.6vw, 1.3rem)' }}>${product.price}</span>
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            <div style={{ background: 'rgba(255,107,0,0.1)', padding: 'clamp(20px, 4vw, 30px)', borderRadius: '15px', border: '2px solid #FF6B00', marginTop: '25px' }}>
              ...
              <p style={{ color: '#DDD6B8', fontSize: 'clamp(1rem, 2.8vw, 1.2rem)', lineHeight: '1.8' }}>
                While harvesting sea moss, our dive team actively restores dying coral reefs. Sea moss and coral have a symbiotic relationship - the moss protects the substrate while encouraging coral growth. We work with both vital coral and ghost coral, replanting with sea moss to restore St. Lucia's volcanic reefs. Every jar you purchase supports this mission.
              </p>
            </div>
          </div>
        </div>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ background: 'rgba(148,0,211,0.2)', padding: 'clamp(20px, 4vw, 30px)', borderRadius: '15px', marginBottom: '25px' }}>
              ...
              <p style={{ color: '#DDD6B8', fontSize: 'clamp(1rem, 2.8vw, 1.2rem)', lineHeight: '1.8', marginBottom: '15px' }}>
                Disease hides behind heavy metals. When you think you've flushed your system but heavy metals remain, you're still transporting disease. To remove disease and all its remnants, you MUST remove the heavy metals, then replenish.
              </p>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ background: 'rgba(255,215,0,0.2)', padding: 'clamp(20px, 4vw, 30px)', borderRadius: '15px', border: '2px solid #FFD700', marginBottom: '25px' }}>
              ...
              <p style={{ color: '#00CED1', fontSize: 'clamp(1rem, 2.8vw, 1.2rem)', lineHeight: '1.8' }}>
                If we're not directly planting seeds from the fruits we consume, we're saving them. Now that we have land, we plant directly - but it's still wise to put some aside. Seeds are sovereignty. Your garden is freedom.
              </p>
            </div>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(255,107,0,0.1), rgba(0,0,0,0.3))', border: '3px solid #FF6B00', borderRadius: '20px', padding: 'clamp(30px, 6vw, 50px)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ background: 'rgba(255,107,0,0.2)', padding: 'clamp(20px, 4vw, 30px)', borderRadius: '15px', marginBottom: '25px' }}>
              ...
              <p style={{ color: '#DDD6B8', fontSize: 'clamp(1rem, 2.8vw, 1.2rem)', lineHeight: '1.8', marginBottom: '15px' }}>
                Castor seeds roasted in St. Lucian sun for 90+ days. Crushed with live electrical coconut oil in a completely holistic, high-vibrational ceremony. This Genesis Batch is more expensive than most castor oil - but the process from seed to bottle creates exponential healing energy.
              </p>
              <p style={{ color: '#00CED1', fontSize: 'clamp(1rem, 2.8vw, 1.2rem)', lineHeight: '1.8' }}>
                Sun and moon fermentation charges the oil with both solar life force and lunar regenerative frequencies. Black seed + oregano infusion = ultimate powerhouse combination.
              </p>
            </div>
      ...
      {/* CHAPTER 7: CONVERGENCE + J2E (TRIPLE PHOTO) */}
      <div style={{ maxWidth: '1200px', margin: '0 auto 80px', padding: '0 20px' }}>
        <div style={{ background: 'linear-gradient(135deg, rgba(0,206,209,0.1), rgba(148,0,211,0.1))', border: '3px solid', borderImage: 'linear-gradient(45deg,violet,indigo,blue,green,yellow,orange,red) 1', borderRadius: '20px', padding: 'clamp(30px, 6vw, 50px)' }}>
          ...
          <p style={{ fontSize: 'clamp(1.1rem, 3vw, 1.4rem)', lineHeight: '1.8', color: '#DDD6B8', marginBottom: '20px' }}>
            Once you've completed your reset, you want to be around people elevated in frequency like yourself. 100K TikTok family. The Smile Game changed lives. Now we converge.
          </p>
          
          {/* J2E CARD WITH TRIPLE PHOTO */}
